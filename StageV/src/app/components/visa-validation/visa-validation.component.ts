...
@Component({
  selector: 'app-visa-validation',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './visa-validation.component.html',
  styleUrls: ['./visa-validation.component.css']
})
export class VisaValidationComponent implements OnInit {
...
// ✅ rôle
  ...
  validateStep1(visaId: number | undefined, approved: boolean) {
    if (!this.canValidate()) {   // ✅ bloque si pas OFFICER
      this.error = 'Action interdite : seul un OFFICER peut valider/rejeter un visa';
      return;
    }
    ...
    this.visaService.validateStep1({ id: visaId, approved }).subscribe({
      next: (response) => {
        if (response instanceof Blob) {
          response.text().then(text => {
            try {
              const updatedVisa = JSON.parse(text) as VisaRequest;
              this.updateVisaList(visaId, updatedVisa);
            } catch {
      ...
      error: (err: HttpErrorResponse) => {
        console.error('Erreur validation visa', err);
        this.error = 'Erreur lors de la validation';
        this.loading = false;
      }
    });
  }
  private updateVisaList(visaId: number, updatedVisa: VisaRequest) {
    const index = this.visaRequests.findIndex(v => v.id === visaId);
    if (index !== -1) {
      this.visaRequests[index] = updatedVisa;
    }
    this.loadAllVisas(); // recharge après modification
  }
